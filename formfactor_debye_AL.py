...
def formfactor(args):
    ...
    ffq = np.sum(np.divide(np.sin(qrvec), qrvec))
    return ffq
...
if __name__ == '__main__': 
    ...
    paramlist = list(itertools.product(range(1), range(n)))
    ...
    # Pq = np.divide(np.sum(np.array(results).reshape(100, n), axis=1), np.square(n))
