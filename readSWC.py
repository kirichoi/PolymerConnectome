...
plt.scatter(radiussize_inv, 
                    spheredist_AL_sum, color='tab:blue', facecolors='none', alpha=0.5)
...
plt.scatter(radiussize_inv[:53], 
                    spheredist_LH_sum[:53], color='tab:green', facecolors='none', alpha=0.5)


plt.plot(radiussize_inv, fitYD_AL1, lw=2, color='tab:blue')
plt.plot(radiussize_inv[:30], fitYD_AL2[:30], lw=2, linestyle='--', color='tab:blue')
plt.plot(radiussize_inv, fitYD_calyx, lw=2, color='tab:orange')
plt.plot(radiussize_inv, fitYD_LH, lw=2, color='tab:green')
plt.yscale('log')
plt.xscale('log')
...
for b in range(len(radiussize)):
    ...
    for ib in range(len(MorphData.LHdist)):
        inbound_LH = np.where(np.sqrt(np.square(np.array(MorphData.LHdist[ib])[:,0] - LHCM[0]) +
                                    np.square(np.array(MorphData.LHdist[ib])[:,1] - LHCM[1]) +
                                    np.square(np.array(MorphData.LHdist[ib])[:,2] - LHCM[2])) <= radiussize[b])[0]
        spheredist_LH_temp.append(len(inbound_LH))
    
    for ib in range(len(MorphData.ALdist)):
        inbound_AL = np.where(np.sqrt(np.square(np.array(MorphData.ALdist[ib])[:,0] - ALCM[0]) +
                                    np.square(np.array(MorphData.ALdist[ib])[:,1] - ALCM[1]) +
                                    np.square(np.array(MorphData.ALdist[ib])[:,2] - ALCM[2])) <= radiussize[b])[0]    
        spheredist_AL_temp.append(len(inbound_AL))
...
poptD_calyx, pcovD_calyx = scipy.optimize.curve_fit(objFuncGL, 
                                                    np.log10(radiussize_inv[0:25]), 
                                                    np.log10(spheredist_calyx_sum[0:25]), 
                                                    p0=[-0.1, 0.1], 
                                                    maxfev=10000)
perrD_calyx = np.sqrt(np.diag(pcovD_calyx))
...
poptD_AL, pcovD_AL = scipy.optimize.curve_fit(objFuncGL, 
                                              np.log10(radiussize_inv[0:40]), 
                                              np.log10(spheredist_AL_sum[0:40]), 
                                              p0=[-0.1, 0.1], 
                                              maxfev=10000)
...
plt.plot(radiussize_inv, fitYD_AL, lw=2, color='tab:blue')
plt.plot(radiussize_inv, fitYD_calyx, lw=2, color='tab:orange')
plt.plot(radiussize_inv, fitYD_LH, lw=2, color='tab:green')
plt.yscale('log')
plt.xscale('log')
...
for i in range(len(radiussize) - shiftN):
    ...
    poptmxc, pcovmxc = scipy.optimize.curve_fit(objFuncGL, 
                                                ...
                                                np.log10(spheredist_calyx_sum[i:i+shiftN]), 
                                                p0=[1., 0.], 
                                                maxfev=100000)
    ...
    poptmxl, pcovmxl = scipy.optimize.curve_fit(objFuncGL, 
                                                np.log10(radiussize[i:i+shiftN]), 
                                                np.log10(spheredist_LH_sum[i:i+shiftN]), 
                                                p0=[1., 0.], 
                                                maxfev=100000)
    ...
    poptmxa, pcovmxa = scipy.optimize.curve_fit(objFuncGL, 
                                                np.log10(radiussize[i:i+shiftN]), 
                                                np.log10(spheredist_AL_sum[i:i+shiftN]), 
                                                p0=[1., 0.], 
                                                maxfev=100000)
...
# morph_dist_calyx_r_new_avg = scipy.spatial.distance.cdist(morph_dist_calyx_CM_avg, morph_dist_calyx_CM_avg)
# morph_dist_LH_r_new_avg = scipy.spatial.distance.cdist(morph_dist_LH_CM_avg, morph_dist_LH_CM_avg)
# morph_dist_AL_r_new_avg = scipy.spatial.distance.cdist(morph_dist_AL_CM_avg, morph_dist_AL_CM_avg)
...
# ax.spines["bottom"].set_visible(False)
# ax.spines["left"].set_visible(False)
# # plt.savefig(Parameter.outputdir + '/hieR_AL_new_nmetric.pdf', dpi=300, bbox_inches='tight')
# plt.show()
...
# ax.spines["bottom"].set_visible(False)
# ax.spines["left"].set_visible(False)
# # plt.savefig(Parameter.outputdir + '/hieR_calyx_new_nmetric.pdf', dpi=300, bbox_inches='tight')
# plt.show()
...
# ax.spines["bottom"].set_visible(False)
# ax.spines["left"].set_visible(False)
# # plt.savefig(Parameter.outputdir + '/hieR_LH_new_nmetric.pdf', dpi=300, bbox_inches='tight')
# plt.show()

# ind_LH_new = scipy.cluster.hierarchy.fcluster(L_LH_new, 0.5*morph_dist_LH_r_new_avg.max(), 'maxclust')
# columns_LH = R_LH_new['leaves']#[morph_dist_LH_r_new_avg_df.columns.tolist()[i] for i in list((np.argsort(ind_LH_new)))]



glo_list_cluster_new = np.array(glo_list)[columns_AL]

glo_len_cluster_new = np.array(glo_len)[columns_AL]

glo_idx_cluster_new = []
for i in range(len(glo_list)):
    glo_idx_cluster_new.append(glo_lb_idx[np.argwhere(np.array(glo_list)[columns_AL][i] == np.array(glo_list))[0][0]])
...
x = np.arange(len(glo_list))
width = 1.
ax.bar(x, np.array(ALcalyx_corr_new_glo_avg)[columns_AL], width, 
       yerr=np.array(ALcalyx_corr_new_glo_std)[columns_AL], label='Calyx-AL', alpha=0.5, 
       error_kw=dict(ecolor='tab:blue', lw=1, capsize=2, capthick=1))
ax.bar(x, np.array(ALLH_corr_new_glo_avg)[columns_AL], width, 
       yerr=np.array(ALLH_corr_new_glo_std)[columns_AL], label='LH-AL', alpha=0.5, 
       error_kw=dict(ecolor='tab:orange', lw=1, capsize=2, capthick=1))
ax.set_ylabel('Correlation Coefficient', fontsize=15)
ax.set_xticks(x)
